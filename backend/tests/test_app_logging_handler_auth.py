...
MODULE_PATH = Path(__file__).resolve().parents[1] / "app" / "utils" / "logging_handler.py"


def load_handler_module():
    spec = importlib.util.spec_from_file_location("app_logging_handler", MODULE_PATH)
...
def test_app_external_logging_handler_adds_auth_header_when_token_is_set(monkeypatch):
...
def test_app_external_logging_handler_omits_auth_header_when_token_is_unset(monkeypatch):
