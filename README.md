...
- **Centralized Auth Service**: All authentication operations use the shared `auth-microservice` (`https://auth.statex.cz`)
...
**Note**: User authentication data (passwords, email verification) is stored in the centralized `auth-microservice` database. This application only stores application-specific user profile data.
...
- `users` - Application-specific user profile data (preferred_currency, telegram settings, etc.)
  - `id` - User ID (matches auth-microservice user ID)
...
- All existing tables include `user_id` foreign keys (references auth-microservice user ID)
