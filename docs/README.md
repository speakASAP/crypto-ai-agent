# Documentation Index

> 📚 **Main Project**: [README](../README.md)

## 🚀 Getting Started

- [**Startup Guide**](STARTUP_GUIDE.md) - Complete setup and installation instructions
- [**Current Plan**](CURRENT_PLAN.md) - Implementation status and roadmap
- [**Technical Implementation**](TECHNICAL_IMPLEMENTATION.md) - Technical architecture details

## 🔐 User Management

- [**User Management**](USER_MANAGEMENT.md) - Multi-user authentication system
- [**Account Management**](CURRENT_PLAN.md#account-management-enhancement-completed) - Account deletion and management

## 📊 Portfolio Management

- [**Multi-Currency Portfolio**](MULTI_CURRENCY_PORTFOLIO_PLAN.md) - Multi-currency support system
- [**Binance Import Guide**](BINANCE_IMPORT_GUIDE.md) - Complete Binance portfolio import documentation
- [**Binance Quick Reference**](BINANCE_QUICK_REFERENCE.md) - Quick setup and troubleshooting
- [**Bitfinex Import Guide**](BITFINEX_IMPORT_GUIDE.md) - Complete Bitfinex portfolio import documentation

## 🚨 Price Alerts

- [**Price Alerts Plan**](PRICE_ALERTS_PLAN.md) - Price alert system implementation
- [**Telegram Integration**](TELEGRAM.md) - Telegram notification setup

## 🔍 Search & Discovery

- [**Crypto Search Guide**](CRYPTO_SEARCH_GUIDE.md) - Cryptocurrency search functionality

## 🛠️ Development & Operations

- [**Logging Configuration**](LOGGING_CONFIG.md) - Centralized logging system
- [**Scripts Documentation**](SCRIPTS_README.md) - Available scripts and utilities
- [**Migration Complete**](MIGRATION_COMPLETE.md) - Database migration status
- [**Docker Deployment**](DEPLOYMENT_DOCKER.md) - Production deployment guide

## 📋 Quick Reference

### Most Common Tasks

1. **First Time Setup**
   - [Startup Guide](STARTUP_GUIDE.md)
   - [User Management](USER_MANAGEMENT.md)

2. **Import Portfolio from Exchanges**
   - [Binance Quick Reference](BINANCE_QUICK_REFERENCE.md)
   - [Binance Import Guide](BINANCE_IMPORT_GUIDE.md)
   - [Bitfinex Import Guide](BITFINEX_IMPORT_GUIDE.md)

3. **Set Up Price Alerts**
   - [Price Alerts Plan](PRICE_ALERTS_PLAN.md)
   - [Telegram Integration](TELEGRAM.md)

4. **Troubleshooting**
   - [Logging Configuration](LOGGING_CONFIG.md)
   - [Technical Implementation](TECHNICAL_IMPLEMENTATION.md)

### API Documentation

- **Backend API**: <http://localhost:8000/docs>
- **Frontend**: <http://localhost:3000>
- **WebSocket**: ws://localhost:8000/ws

## 🆕 Recent Updates

### Latest Features (2025-01-25)

- ✅ **Bitfinex Portfolio Import** - Automatically import your Bitfinex holdings
- ✅ **Binance Portfolio Import** - Automatically import your Binance holdings
- ✅ **Multi-Currency Support** - USD, EUR, CZK tracking
- ✅ **Source Tracking** - Track where assets were purchased
- ✅ **Import History** - Complete import tracking

### Previous Updates

- ✅ **Account Management** - Secure account deletion
- ✅ **Price Alerts** - Real-time price notifications
- ✅ **User Authentication** - Multi-user system
- ✅ **Portfolio Management** - Real-time tracking

---

**Need Help?** Check the [main README](../README.md) or review the specific documentation for your use case.
